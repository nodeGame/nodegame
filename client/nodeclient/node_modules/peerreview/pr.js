...
function PeerReview () {
	...
	var evaluation = function(){
		...
		var faces = this.memory.getByKey('CF').fetch();
//		console.log('FACES');
//		console.log(faces);
...
//		node.log(matches);
	...
	var dissemination = function(){
		...
		for (var i=0; i < exhibs.length; i++) {
			...
			var works = exhibs[i].groupBy('value.EVA.for');
			
			// Evaluations Loop
			for (var j=0; j < works.length; j++) {
	
				var mean = works[j].mean('value.EVA.eva'); 
				
				// Threshold
				if (mean > this.threshold) {	
					
					var player = works[j].fetch()[0].player;
					var cf = this.memory.getByState(this.previous(3))
										.getByPlayer(player)
										.getByKey('CF')
										.fetchValues()[0];
					...
					selected.push({ex: works[j].fetch()[0].value.SUB,
								   mean: mean,
								   author: player,
								   cf: cf});
				}
