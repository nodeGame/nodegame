...
function Game (settings,gamesocketclient) {
	...
	var incomingListeners = function() {
		...
		node.on( IN + set + 'STATE', function(p,msg){
		...
		node.on( IN + say + 'PLIST', function(p,msg) {
			...
			if (that.automatic_step) {
				...
				if (morePlayers > 0 ) {
					node.fire('OUT.say.MSG', morePlayers + ' player/s still needed to begin the game');
				...
				// TODO: differentiate between before the game starts and during the game
					...
					node.fire('OUT.say.MSG', this.minPlayers + ' players connected. Game can start');
...
//			}
	...
	var outgoingListeners = function() {
		...
		node.on( OUT + set + 'STATE', function(p,action,state,to){
			//console.log('BBBB' + p + ' ' + args[0] + ' ' + args[1] + ' ' + args[2]);
			node.gsc.sendSTATE(action,state,to);
		});		
	
		// SAY
		
		node.on( OUT + say + 'STATE', function(p,action,state,to){
			//console.log('BBBB' + p + ' ' + args[0] + ' ' + args[1] + ' ' + args[2]);
			node.gsc.sendSTATE(action,state,to);
		});	
		
		node.on( OUT + say + 'MSG', function(p,text,to){
			node.gsc.sendMSG(text,to);
		});
		
		node.on( OUT + say + 'DATA', function(p,data,to,msg){
			node.gsc.sendDATA(data,to,msg);
		});
		
		node.on("DONE", function(p,d){
			that.is(GameState.iss.DONE);
			that.publishState();
		});
		
		node.on('WAIT', function(p){
...
Game.prototype.STATE = function(action,state,to){
	...
	node.fire(stateEvent,action,state,to);
};
...
Game.prototype.DATA = function(data,to,msg) {
	node.fire("out.say.DATA",data,to,msg);
};
