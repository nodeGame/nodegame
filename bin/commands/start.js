...
module.exports = function (program, rootDir) {
    ...
    program
        .command("start")
        .description("Starts the nodeGame server")
        ...
        .option("-c, --confDir <confDir>", "Sets the configuration directory")
        .option("-l, --logDir <logDir>", "Sets the log directory")
        .option(
            "-L, --logLevel <logDir>",
            "Sets the log level. Values: error(default)|warn|info|silly"
        )
        .option("-g, --gamesDir <gamesDir>", "Sets the games directory")
        .option("-d, --debug", "Enables the debug mode")
        .option(
            "-i, --infoQuery [false]",
        ...
        .option(
            "-b, --build [components]",
            "Rebuilds the specified components",
            list
        )
        .option(
            "-s, --ssl [path-to-ssl-dir]",
    ...
    function startServer() {
        ...
        options.nodeGameRoot = rootDir;

        // Start server, options parameter is optional.
        const sn = new ServerNode(options);
        ...
        sn.ready(function () {
            ...
            if (auth && !channel.gameInfo.auth.enabled) {
                printErr(
                    "auth option enabled, but channel does not support it."
                );
                process.exit();
            }
    ...
    function writeSettingsFile(gameDir) {
        ...
        fs.writeFileSync(
            path.resolve(gameDir, "test", "settings.js"),
            settings
        );
    }
};
